"""
Groq Adapter - Implementation for Groq API with DeepSeek-R1-Distill-Llama-70B.
High-velocity inference for faster Tree-of-Thought exploration.

Key differences from OpenAI adapter:
1. Uses Groq API endpoint
2. NO system prompts (distilled models ignore them) - instructions in user message
3. Much faster inference (~300+ tokens/sec vs ~50)
"""
...
class GroqAdapter(BaseLLMAdapter):
    """
    Groq adapter with DeepSeek-R1-Distill-Llama-70B.
    
    Key characteristics:
    - Very fast inference (300+ tokens/sec)
    - Trained on reasoning traces
    - IMPORTANT: Does NOT use system prompts (they are ignored)
    ...
    def __init__(self, api_key: str = None, model: str = "openai/gpt-oss-120b"):
        ...
        if not self.api_key:
            raise ValueError(
                ...
                "Please set GROQ_API_KEY environment variable or pass api_key parameter."
            )
    ...
    def _call_llm(self, user_prompt: str, max_tokens: int = 8000, json_mode: bool = False) -> str:
        """
        Make a call to Groq API.
        
        IMPORTANT: No system prompt! Distilled models ignore system prompts.
        All instructions must be in the user message.
        ...
        try:
            logger.info(f"Making Groq API call with model: {self.model}, json_mode: {json_mode}")
            logger.info(f"Request params keys: {list(request_params.keys())}")
            response = self.client.chat.completions.create(**request_params)
            logger.info(f"Groq API call successful, response received")
    ...
    def generate_hypotheses(self, problem_summary: str) -> Dict:
        """Generate initial hypotheses using Groq/DeepSeek."""
        try:
            # NOTE: Instructions at the TOP of user message (no system prompt)
            user_prompt = f"""INSTRUCTIONS: You are an expert MongoDB consultant. Provide structured, actionable analysis.

TASK: You are helping to debug a MongoDB performance issue.
...
..."""
            ...
            # Fallback parsing if no hypotheses found - try more flexible patterns
            if not hypotheses:
                ...
                for i, line in enumerate(lines):
                    ...
                    if (line and (line[0].isdigit() or line.startswith('-') or line.startswith('*')) 
                        ...
                        # Try various regex patterns (on cleaned line without markdown)
                        patterns = [
                            r'(.+?)\s*\|\s*Category:\s*([^|]+?)\s*\|\s*Confidence:\s*([\d.]+)',  # Standard format
                            r'(.+?)\s*-\s*Category:\s*([^|]+?)\s*-\s*Confidence:\s*([\d.]+)',  # Dashes as separators
                            r'(.+?)\s*Category:\s*([^|]+?)\s*Confidence:\s*([\d.]+)',  # No separators
                        ]
                        
                        # Use cleaned line for pattern matching
                        line_to_match = line_clean
                        
                        for pattern in patterns:
                            match = re.search(pattern, line_to_match, re.IGNORECASE)
                            if match:
                                desc = match.group(1).strip().lstrip('0123456789.-* )').strip()
                                # Remove any remaining markdown from description
                                desc = re.sub(r'\*\*([^*]+)\*\*', r'\1', desc)
            ...
            logger.error(f"Groq hypothesis generation failed: {e}", exc_info=True)
    ...
    def evaluate_branches(
        ...
        """Evaluate multiple hypothesis branches using Groq/DeepSeek."""
        logger.info(f"GroqAdapter.evaluate_branches called with {len(branch_nodes)} branches")
        try:
            ...
            # NOTE: Instructions at the TOP of user message (no system prompt)
            # Bayesian Evidence Screening: Request likelihood scores instead of confidence
            user_prompt = f"""INSTRUCTIONS: You are an expert MongoDB consultant. Evaluate evidence using Bayesian likelihood ratios.

TASK: Determine if the evidence SUPPORTS or REFUTES each hypothesis branch.
...
Be specific about the causal logical link between evidence and hypothesis."""
            ...
            logger.info(f"Calling Groq LLM with JSON mode for {len(branch_nodes)} branches")
            llm_output = self._call_llm(user_prompt, json_mode=True)
            ...
            logger.error(f"Groq branch evaluation failed: {e}", exc_info=True)
    ...
    def generate_next_requests(
        ...
        """Generate next data requests using Groq/DeepSeek."""
        try:
            ...
            # NOTE: Instructions at the TOP of user message (no system prompt)
            user_prompt = f"""INSTRUCTIONS: You are an expert MongoDB consultant. Provide specific, actionable data requests.

TASK: Determine what diagnostic data to request next.
...
Be specific about what commands to run or what outputs to collect."""
            ...
            logger.error(f"Groq next requests generation failed: {e}", exc_info=True)
            raise ValueError(f"LLM request generation failed: {e}")
    
    def generate_final_analysis(self, engine: 'TreeOfThoughtEngine') -> Dict:
        """Generate final root cause analysis using Groq/DeepSeek."""
        try:
            ...
            if all_pruned or not top_hypotheses:
                ...
                user_prompt = f"""INSTRUCTIONS: You are an expert MongoDB consultant providing comprehensive root cause analysis.

TASK: The debugging session has collected some artifacts, but all initial hypotheses were pruned (ruled out) based on the available data.
...
TASK: Provide the final root cause analysis for a MongoDB performance issue.
...
Format your response clearly with these sections. Be specific and actionable."""
            ...
            logger.error(f"Groq final analysis failed: {e}", exc_info=True)
    ...
    def generate_sub_hypotheses(
        ...
        """Generate sub-hypotheses from a node using Groq/DeepSeek."""
        try:
...
"""
            ...
            # NOTE: Instructions at the TOP of user message (no system prompt)
            user_prompt = f"""INSTRUCTIONS: You are an expert MongoDB consultant. Generate focused, actionable exploration directions from specific points in a reasoning tree.

TASK: Explore a debugging scenario from a specific point in the reasoning tree.
...
..."""
            ...
            logger.error(f"Groq node expansion failed: {e}", exc_info=True)

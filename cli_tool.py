...
class CLIDebuggingTool:
    ...
    def _display_current_state(self):
        ...
        summary_table.add_row("Step", str(node.step_number))
        summary_table.add_row("Total Nodes", str(summary['total_nodes']))
        summary_table.add_row("Active Hypotheses", str(summary['active_hypotheses']))
        ...
        # Hypotheses table
        hyp_table = Table(title="Active Hypotheses", show_header=True, header_style="bold cyan")
        ...
        for h in node.hypotheses:
            if h.status == "active":
                ...
                hyp_table.add_row(
                    ...
                    f"[{'green' if h.status == 'active' else 'red'}]{h.status}[/{'green' if h.status == 'active' else 'red'}]"
                )
        ...
        # Next requests
        next_requests = self.engine.get_next_requests(node)
        if next_requests:
    ...
    def _upload_artifact(self):
        ...
        with Progress(
            ...
            task = progress.add_task("Processing artifact...", total=None)
            
            try:
                # LLM evaluation is now handled inside add_artifact()
                new_node = self.engine.add_artifact(artifact_name, artifact_content)
        ...
        self.console.print("\n[green]✓ Artifact processed![/green]")
        
        # Show evidence found
        evidence_found = []
        for h in new_node.hypotheses:
            if h.evidence:
                evidence_found.extend([(h.description, e) for e in h.evidence[-3:]])
    ...
    def _backtrack(self):
        """Handle backtracking to previous nodes."""
        node = self.engine.get_current_node()
        
        if not node.parent_id:
            self.console.print("[yellow]No parent node to backtrack to.[/yellow]")
            return
